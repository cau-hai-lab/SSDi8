...
from utils import (
    ...
    build_mamba_and_tokenizer_hybrid, 
    set_deterministic, 
    parse_options
)
import logging
import sys
import os
from ssdi8.eval_utils import eval_mamba_few_shot,evaluate_ppl
...
def main(args):    
    ...
    model, tokenizer, is_ssdi8 = build_mamba_and_tokenizer(args, model_type)
    ...
    if args.log_dir:
        ...
        if args.quantize:
            log_name = f"{model_name}" if is_ssdi8 else f"{model_name}_w{args.w_bits}a{args.a_bits}"
